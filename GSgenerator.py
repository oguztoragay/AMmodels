""" Created on Tue May 12 19:02:12 2020 
    ...
    OOP is used in theis version of the code 
    @author: ozt0008 
...
##--------------------------------------------------------------------------------------        
class Generate: # Groun structure generator
#    r_set = [] # set of available discrete circular cross-sectional radius
    def __init__(self, M, N, fix_nodes, load_nodes, load_values,E,r_set):
        ...
        self.celements = {} # Dictionary of continiuous Element instances to be filled
...
class Profile: 
    def __init__(self,el, E, r):
        ...
        if r != 0:
            ...
            self.b2g = np.dot(el.Tmat,np.dot(el.transform.T,np.array([[0, 2/el.length, 1, 0, -2/el.length, 1]]).T))
    ...
    def _lmatrix(self):
        ...
        if self.r != 0:
            ...
            b2loc = np.dot(np.array([[0, 2/self.el.length, 1, 0, -2/self.el.length, 1]]).T, np.array([[0, 2/self.el.length, 1, 0, -2/self.el.length, 1]]))
    ...
    def _B(self):
        B = []
        B.append(self.b1g.T.flatten())
        B.append(self.b2g.T.flatten())
        B.append(self.b3g.T.flatten())
        return B 
##-------------------------------------------------------------------------------------- 
class CElement:
    def __init__(self, nodei, nodej, M, N, elem, E):
        ...
        self.b2g = np.dot(self.Tmat,np.dot(self.transform.T,np.array([[0, 2/self.length, 1, 0, -2/self.length, 1]]).T))
...
#        return ctemp
    ...
    def _Bloc(self):
        b1loc = np.dot(np.array([[-1, 0, 0, 1, 0, 0]]).T, np.array([[-1, 0, 0, 1, 0, 0]]))
        b2loc = np.dot(np.array([[0, 2/self.length, 1, 0, -2/self.length, 1]]).T, np.array([[0, 2/self.length, 1, 0, -2/self.length, 1]]))
        b3loc = np.dot(np.array([[0, 0, -1, 0, 0, 1]]).T, np.array([[0, 0, -1, 0, 0, 1]]))
