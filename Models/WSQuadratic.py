...
# %% Defining Sets, Variables and Parameters
def GBNLPpyo(E, nodes, celements, r2_set, dmax, smax, Xw, Yw, Dw, Vw, Sw):
    ...
    m.setParam('TimeLimit', 3600)  # Time limit increase from 10800 to 18000 on 11.23.2020
    ...
    m.setParam('MIPFocus', 2) # added on 11/1/2021
    # m.setParam('Method', 1)
    ############################################################### added on 02/02/2021   CHECK THESE OUT
#    m.setParam('MIQCPMethod', 0) # 1 uses a linearized, outer-approximation approach, while value 0 solves continuous QCP relaxations at each node.
#    m.setParam('PreMIQCPForm', 0)   # 0 leaves the model in MIQCP form, so the branch-and-cut algorithm will operate on a model with arbitrary quadratic constraints.
                                    # 1 always transforms the model into MISOCP form; quadratic constraints are transformed into second-order cone constraints.
                                    # 2 always transforms the model into disaggregated MISOCP form; quadratic constraints are transformed into rotated cone constraints, 
    ...
    # -----------------------------------------------------------------------------------------
    for i in LE:
        m.addQConstr(ar_2[i] <= ar[i] * ar[i], name='PowerOfAreas1')
        m.addQConstr(ar_2[i] >= ar[i] * ar[i], name='PowerOfAreas2')
    # -----------------------------------------------------------------------------------------
    ...
    for i in LE:  # Removing the cross-overing beams
        for j in range(i + 1, len(LE)):
            ...
            if int_pt == True and toucher == False:
                m.addQConstr(ar[i] * ar[j] <= 0, name='Crossing1')
                m.addQConstr(ar[i] * ar[j] >= 0, name='Crossing2')
    m.update()
    for i in LE:
        if Xw[i, 1] == 1:
            ar[i].setAttr('Start', amax) #0.785398
            ar_2[i].setAttr('Start', amax**2) #0.616850
        else:
...
# %% Defining Sets, Variables and Parameters
def QUAD_without(E, nodes, celements, r2_set, dmax, smax):
    boundary = []
    load_node = []
    nfree = []
    load_node_dof = []
    ...
    # m.setParam('Method', 1)
    ############################################################### added on 02/02/2021   CHECK THESE OUT
#    m.setParam('MIQCPMethod', 1) # 1 uses a linearized, outer-approximation approach, while value 0 solves continuous QCP relaxations at each node.
#    m.setParam('PreMIQCPForm', 2)   # 0 leaves the model in MIQCP form, so the branch-and-cut algorithm will operate on a model with arbitrary quadratic constraints.
                                    # 1 always transforms the model into MISOCP form; quadratic constraints are transformed into second-order cone constraints.
                                    # 2 always transforms the model into disaggregated MISOCP form; quadratic constraints are transformed into rotated cone constraints, 
                                    # where each rotated cone contains two terms and involves only three variables.
    LN = list(nodes.keys())
    ...
    QPstart1 = time.time()
    m.optimize()
    TQP1 = np.round(time.time() - QPstart1, 3)
    data_q = [m.ObjBound, m.ObjBoundC, m.ObjVal, m.MIPGap, m.Runtime]
    Y = []
    weight1 = 0
    if m.Status != 2:
        print(m.Status)
